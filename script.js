(() => {
  ...
  const bootLines = [
    ['[  OK  ] ', 'Reached target Phosphor Display.',       'ok'],
    ['[  OK  ] ', 'Loaded kernel sarfaraz-6.4.0-matrix.',   'ok'],
    ['[  OK  ] ', 'Mounted /dev/skills on /home/sarfaraz.', 'ok'],
    ['[  OK  ] ', 'Started AWS-cloud-sync.service.',        'ok'],
    ['[ WARN ] ', 'Coffee daemon running at 42% capacity.', 'warn'],
    ['[  OK  ] ', 'TypeScript compiler online.',            'ok'],
    ['[  OK  ] ', 'Connected to Matrix mainframe @ 8.8.8.8.', 'ok'],
  ...
  if (fxBtn) {
    fxBtn.addEventListener('click', () => {
      ...
      if (off) stopRain(); else startRain();
    });
  }
...
linkedin : linkedin.com/in/sk-sarfaraz`,
    ...
    phosphor: () => { themeBtn && themeBtn.click(); return 'phosphor color toggled.'; },
    clear:    () => { history.innerHTML = ''; history.classList.add('hidden'); return null; },
    ls:       () => `about.md  skills/  experience/  projects/  resume.pdf  contact.sh`,
    pwd:      () => '/home/sarfaraz',
    uname:    () => 'sarfaraz-6.4.0-matrix x86_64',
  };
  ...
  function runCommand(raw) {
    ...
    block.innerHTML = `<div style="margin-top:10px">${promptHtml}</div>` +
                      (out == null ? '' : `<pre style="margin:4px 0 0;padding:4px 0 4px 14px;border-left:2px solid var(--green-dark);white-space:pre-wrap;color:var(--green-pale)">${out}</pre>`);
